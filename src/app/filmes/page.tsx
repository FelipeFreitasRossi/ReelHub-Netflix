...
export default function FilmesPage() {
  ...
  // Filtra as categorias que contêm filmes e mantém apenas os itens do tipo "Filme"
  const filmesCategories = categories
    .map((cat) => ({
      ...cat,
      items: cat.items.filter((movie) => movie.type === "Filme"),
    }))
    .filter((cat) => cat.items.length > 0);

  // Pega o primeiro item da primeira categoria para o banner
  const featuredFilme = filmesCategories.length > 0 ? filmesCategories[0].items[0] : null;
    ...
    return (
      <div className="bg-black min-h-screen text-white">
        ...
        <main className="pt-24 px-4 md:px-12">
          <h1 className="text-3xl md:text-4xl font-bold">Filmes</h1>
          <p className="text-gray-400 mt-4">Nenhum filme encontrado no momento.</p>
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="relative bg-black min-h-screen text-white overflow-x-hidden">
      ...
      <motion.section
        ...
        style={{ backgroundImage: `url(${featuredFilme.backdrop})` }}
      >
        ...
        <div className="relative z-10 max-w-2xl px-4 md:px-12 space-y-4">
          ...
          >
            ...
            <span className="text-sm text-gray-300">{featuredFilme.year}</span>
            <span className="text-sm text-gray-300 flex items-center gap-1">
              ⭐ {featuredFilme.rating}
            </span>
            <span className="text-sm text-gray-300 border border-gray-500 px-1 rounded text-xs">
              {featuredFilme.maturity === "L" ? "LIVRE" : `${featuredFilme.maturity}+`}
            </span>
          </motion.div>
          ...
          >
            <button
              onClick={() => {
                setSelectedMovie(featuredFilme);
                setIsModalOpen(true);
              }}
      ...
      {/* ===== LINHAS DE FILMES POR CATEGORIA ===== */}
